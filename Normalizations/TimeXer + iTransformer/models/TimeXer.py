...
class TimeXer(nn.Module):
    ...
    def forecast_multi(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        if self.use_norm:
            ...
            #print("x:",x_enc.shape) #B, T, C x: torch.Size([32, 36, 7])
            # Normalization from Non-stationary Transformer
#             means = x_enc.mean(1, keepdim=True).detach()
#             x_enc = x_enc - means
#             stdev = torch.sqrt(torch.var(x_enc, dim=1, keepdim=True, unbiased=False) + 1e-5)
...
#         if self.use_norm:
# #             print("dec-out before denorm", dec_out.shape) dec-out before denorm torch.Size([32, 6, 7]) #B, T, C

#             # De-Normalization from Non-stationary Transformer
#             dec_out = dec_out * (stdev[:, 0, :].unsqueeze(1).repeat(1, self.pred_len, 1))
#             dec_out = dec_out + (means[:, 0, :].unsqueeze(1).repeat(1, self.pred_len, 1))
#             #print("dec-out after denorm", dec_out.shape) dec-out after denorm torch.Size([32, 6, 7]) #B, T, C
