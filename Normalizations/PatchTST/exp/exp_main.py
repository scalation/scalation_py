...
class Exp_Main(Exp_Basic):
    ...
    def residual_diagnostics(self,resid, fitted, label=""):
        results = {}
        results["Variance"] = np.var(resid, ddof=1)
        results["Skewness"] = skew(resid)
        results["Kurtosis"] = kurtosis(resid, fisher=False)
        results["Corr(resid, fitted)"] = np.corrcoef(fitted, resid)[0,1]
        results["Shapiro-W p"] = shapiro(resid)[1]
...
#                     if "gate_tau" in name or "gate_slope" in name:
#                         print(f"{name}: {param.data.cpu().numpy()}")
            ...
            # ---- Report to Ray (safe if not running under Ray) ----
            try:
                from ray.air import session as _ray_session
                _ray_session.report({"val_loss": vali_loss, "training_iteration": epoch + 1})
            except Exception:
                pass
...
#                 epoch + 1, train_steps, train_loss, vali_loss, test_loss))
        ...
        best_model_path = path + '/' + 'checkpoint.pth' #os.path.join(path, 'checkpoint.pth')
...
#             print("before",trues[:,:,4])
            ...
            # x positions
            x_true = np.arange(len(true_data))
            x_pred = np.arange(len(true_data))  
            ...
            plt.plot(x_true, true_data, color='tab:orange', label='True Data')
            plt.plot(x_pred, pred_data, color='tab:blue', label='Predictions')
            ...
            plt.legend()
            plt.grid(False)
            plt.title(f"{self.args.scale_method}", fontsize=10, fontweight="bold")
