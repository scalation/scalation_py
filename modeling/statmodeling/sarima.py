...
class SARIMA(Model):
    def __init__(self, args):
        ...
        self.model_name = f'SARIMA({self.p},{self.d},{self.q},{self.P},{self.D},{self.Q},{self.s}) rc = {self.rc}'
        super().__init__(args)
        ...
        for i in tqdm(range(self.n_features)):
            for j in tqdm(range(self.test_size - sample_offset)):
                if j % self.rc ==0:
                    ...
                    sarima_model = SARIMAX_(train_data,
                                            order=(self.p, self.d, self.q),
                                            seasonal_order=(self.P, self.D, self.Q, self.s),
                                            trend=self.trend,
                                            enforce_stationarity=False, enforce_invertibility=False).fit(method = self.fit_method)
                else:
                    if self.skip_insample is None:
                        new_data = self.data.iloc[self.train_size + j - 1:self.train_size + j, i]
                        train_data = pd.concat([train_data, new_data])
                        sarima_model = sarima_model.append(endog=new_data, refit=False)

                start = self.train_size + j
                end = start + self.pred_len - 1

                forecasts = sarima_model.predict(start = start, end = end, dynamic = True).values.T.reshape(1, self.pred_len)
                np.fill_diagonal(self.forecast_tensor[j:, :, i], forecasts)


        self.total_params = len(sarima_model.params.index[sarima_model.params.index != 'sigma2'])
