...
from modeling.neuralmodeling.layers.Embed import DataEmbedding_inverted, PositionalEmbedding
import numpy as np
...
class EnEmbedding(nn.Module):
    def __init__(self, n_vars, d_model, patch_len, stride, dropout):
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        # Embedding
        self.en_embedding = EnEmbedding(self.n_vars, configs['d_model'], self.patch_len, self.patch_len, configs['dropout'])
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        if self.use_norm:
            # transformation from Non-stationary Transformer
        ...
        if self.use_norm:
            # De-transformation from Non-stationary Transformer
