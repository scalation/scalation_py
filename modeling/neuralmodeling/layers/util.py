...
# transformation, pointwise gaussian
...
# transformation, scaling by range
...
class CustomLayerNorm(nn.Module):
    def __init__(self, normalized_shape, eps=1e-5, elementwise_affine=True):
        ...
        if self.elementwise_affine:
            # normalized_shape could be a single integer or a tuple of integers.
            # Ensure it is a tuple for consistent handling.
            if isinstance(normalized_shape, int):
                normalized_shape = (normalized_shape,)
            else:
                normalized_shape = tuple(normalized_shape)

            # Initialize gamma (scale) and beta (shift) as learnable parameters
    ...
    def forward(self, x):
        # Compute mean and standard deviation over the specified dimensions
        ...
        # Scale and shift if affine parameters are enabled
        if self.elementwise_affine:
