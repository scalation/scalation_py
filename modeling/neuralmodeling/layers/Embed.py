...
class TimeDelayMultiViewEncEmbedding(nn.Module):
    def __init__(self, d_model, seq_len, seg_len, stride, dropout_embed, freq, num_views=8, time_delays=3):
        super(TimeDelayMultiViewEncEmbedding, self).__init__()
        self.seg_len = seg_len
        self.stride = stride
        self.d_model = d_model
        ...
        if d_model % num_views != 0:
            raise ValueError("d_model must be divisible by num_views to ensure even distribution.")

        self.linears = nn.ModuleList([
            nn.Linear(seg_len * time_delays, d_model // num_views) for _ in range(num_views)
        ])
        self.dropout = nn.Dropout(p=dropout_embed)
    ...
    def forward(self, x, x_mark):
        ...
        print(x.shape)
        delayed_segments = []
        max_delay_length = x.size(1) - self.time_delays + 1
        for delay in range(self.time_delays):
            shifted_x = torch.roll(x, shifts=-delay, dims=1)
            shifted_x = shifted_x[:, :max_delay_length, :, :]
            print('shifted_x: ', shifted_x.shape)
            delayed_segments.append(shifted_x)
        x = torch.cat(delayed_segments, dim=2)
        view_embeddings = []
        for linear in self.linears:
            view_embedding = linear(x.view(batch, -1, self.seg_len * self.time_delays))
            view_embeddings.append(view_embedding)
        x_embed = torch.cat(view_embeddings, dim=-1)
        x_embed = self.dropout(x_embed)
        print(x_embed.shape)
        return (x_embed.unsqueeze(1))
...
class enc_embedding(nn.Module):
    ...
    def forward(self, x, x_mark):
        ...
        x = x.unfold(dimension = -1, size = self.seg_len, step = self.stride)
        x_embed = self.linear_x_embed(x)
        temp1, temp2, temp3, temp4, temp5, temp6 = self.temporal_embedding(x_mark)
        return self.dropout(x_embed + temp1 + temp2 + temp3 + temp4 + temp5 + temp6 + pe)
"""