...
def get_metrics(actual, forecasts) -> Tuple[int, float, float, float]:
    """
...
def diagnose(self, forecasts, forecasts_original) -> None:
    ...
    for h in self.args.horizons:
        ...
        N, mse_normalized, mae_normalized, smape_normalized = get_metrics(
            actual, forecasts[start_index:end_index, self.target_int, h - 1])
        N, mse_original, mae_original, smape_original = get_metrics(
            actual_original, forecasts_original[start_index:end_index, self.target_int, h - 1])
