...
class Exp_Main(Exp_Basic):
    ...
    def train(self, setting):
        ...
        vali_data, vali_loader = self._get_data(flag='train')#for covid, 'val' for ILI
...
#         print('test shape:', preds.shape, trues.shape)
        ...
        N = self.args.pred_len
        RealSMAPE = [0]*N
        RealMAE = [0]*N
        mae, mse, rmse, mape, mspe = [[0]*N for l in range(5)]
        for horizon in range(N):
            ppreds = preds[:, horizon, 0]
            ttrues = trues[:, horizon, 0]
            mae[horizon], mse[horizon], rmse[horizon], mape[horizon], mspe[horizon] = metric(ppreds, ttrues)
      
            RealSMAPE[horizon] = metric(test_data.inverse_transform(preds[:, horizon, :])[:, 0],
                                        test_data.inverse_transform(trues[:, horizon, :])[:, 0])[3]
            RealMAE[horizon] = metric(test_data.inverse_transform(preds[:, horizon, :])[:, 0],
                                        test_data.inverse_transform(trues[:, horizon, :])[:, 0])[0]
