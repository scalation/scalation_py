...
class AR(Model):
    ...
    def train_test(self) -> Tuple[np.array, np.array]:
        ...
        def recursive_forecast(self, history, params, max_horizons):
            ...
            for i in range(max_horizons):
                ...
                for j in range(self.args.p):
                    forecast += params.get(f'{self.args.target}.L{j + 1}', 0) * forecasts[-j - 1]
        ...
        for i in range(len(self.train_data.columns) - 1):
            ...
            for j in range(len(self.test_data) + self.max_horizon):
                ...
                predictions = recursive_forecast(self,
                                                 data_temp,
                                                 params,
