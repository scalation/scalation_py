...
from utils.tools import logtransform, sqrttransform
...
class Dataset_Custom(Dataset):
    def __init__(self, root_path, flag='train', size=None,
                 features='S', data_path='ETTh1.csv',
                 target='OT', scale=True, timeenc=0, freq='h',scale_method='log',
                 difforder = 'Seasonal',seasonal = 13, difference = False):
        # size [seq_len, label_len, pred_len]
        # info
        if size == None:
            self.seq_len = 24 * 4 * 4
...
#                 print(self.scaler.lambdas_)
                ...
                data = self.scaler.transform(df_data.values)
            elif self.scale_method == 'logpos':
                self.scaler = logtransform_forpositive()
                data = self.scaler.transform(df_data.values)
        else:
            data = df_data.values
