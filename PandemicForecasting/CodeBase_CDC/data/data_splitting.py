...
def train_test_split(x: np.ndarray, y: np.ndarray, test_ratio: float = 0.25) -> (
...
def make_input_output_sequences(series, n_past, n_future, include_dates):
    ...
    if include_dates is True:
        ...
        return torch.from_numpy(np.array(x[:, :, 1:], dtype=np.float32)).float().to(device), torch.from_numpy(
            np.array(y[:, :, 1:], dtype=np.float32)).float().to(device)
...
def shift_sequence(x_train, y_train, x_test, y_test, window, include_dates):
    ...
    if include_dates is True:
        ...
        return torch.from_numpy(x_train).to(device).float(), x_test.to(device).float(), torch.from_numpy(y_train).to(
            device).float(), y_test.to(device).float()