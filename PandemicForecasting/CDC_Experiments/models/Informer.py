...
class Informer(nn.Module):
    ...
    def forward(self, x_enc, _, __, enc_self_mask=None, dec_self_mask=None, dec_enc_mask=None):
        ...
        x_dec = torch.cat([x_enc[:, -self.pred_len:, :], torch.zeros_like(x_enc[:, -self.pred_len:, :]).float()],
                          dim=1).float().to(device)

        enc_out = self.encProjection(
            x_enc) + self.encEmbedding  # self.enc_embedding(x_enc, x_mark_enc) # batch, length, channel
